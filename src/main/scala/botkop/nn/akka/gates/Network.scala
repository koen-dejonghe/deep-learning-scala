...
case class Network(gates: List[Gate] = List.empty,
  ...
  def init()(implicit system: ActorSystem): (ActorRef, ActorRef) = {
    ...
    val output = system.actorOf(OutputGate.props(costFunction, maxIterations), "output")
      ...
      gates.reverse.foldLeft(output, dimensions.length) {
        case ((next, i), gate) =>
          gate match {
            ...
            case Dropout =>
              val gate =
                system.actorOf(DropoutGate.props(next, dropout),
                               Dropout.name(i))
              (gate, i)
          }
      }
    ...
    val input =
      system.actorOf(InputGate.props(first, miniBatchSize = miniBatchSize), "input")
